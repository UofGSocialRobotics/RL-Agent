...
class Agent():
    ...
    def next_rl(self, state, epsilon):
        if random.uniform(0, 1) > epsilon:
            agent_action = self.next_best(state.get_state())
            agent_action = utils.recreate_action(agent_action)
        else:
...
#        print("dans letat + " + current_state + " laction la meilleur est " + action)
        return action
